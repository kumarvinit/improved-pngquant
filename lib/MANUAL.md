...
Based on modernized [pngquant](http://pngquant.org).
...
The library itself is written in C99, so requires a not-completely-outdated C compiler. GCC and clang (LLVM) work. MSVC won't.

    gcc -std=c99 -O3 -DNDEBUG lib/*.c yourprogram.c

##Download

The library is currently a part of the [pngquant project](https://github.com/pornel/improved-pngquant). See `lib/` directory inside the repository.
...
Returns `LIQ_VALUE_OUT_OF_RANGE` if number of colors is outside 2-256 range.
...
If it's not possible to convert image with at least `minimum` quality, then `liq_quantize_image()` will fail. The default is `0`.
...
`gamma` can be `0` for images with the typical 1/2.2 gamma. Otherwise `gamma` must be > 0 and < 1, e.g. `0.45455` (1/2.2) or `0.55555` (1/1.8). Generated palette will use the same gamma unless `liq_set_output_gamma()` is used.
...
Passes ownership of bitmap and/or rows memory to the object. Memory owned by the object will be freed at object's discretion with `free` function specified in `liq_attr_create_with_allocator()` (by default it's stdlib's `free()`).
